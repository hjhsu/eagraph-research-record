# Publication review required

The following decisions remain open:

- [x] Confirm authors, order, affiliations, and corresponding email.
- [x] Confirm ORCIDs (Hwai-Jung Hsu: 0000-0002-2586-4484; none supplied for
      the other authors).
- [x] Practitioner contributor Cheng-Jan Chi is included as the second author;
      no separate acknowledgment is used.
- [x] Confirm repository owner and names (`hjhsu/eagraph-benchmark-structure`
      and `hjhsu/eagraph-research-record`).
- [x] Choose licenses (CC BY 4.0 for this research record; Apache-2.0 for the
      benchmark-structure repository; CC BY 4.0 for the paper source).
- [ ] Verify third-party rights.
- [ ] Create an immutable release and DOI; insert it in the paper and both
      citation files.
- [ ] Review the complete public tree for source IDs, local paths, personal
      data, hidden answers, acceptance vectors, and canary values.
- [ ] Verify that public aggregate values agree with the frozen paper source.
- [ ] Compile and visually inspect the arXiv source in TeX Live 2025.
